...
class StoreFileMetadataTestCase(unittest.TestCase):
    ...
    def testInsertFileMetadataSuccessful(self):
        with patch('lambdas.document_manager.services.dynamo_service.boto3.client', return_value=self.dynamo_client):
            from lambdas.document_manager.services.dynamo_service import store_file_metadata
...
class ReadFileMetadataTestCase(unittest.TestCase):
    ...
    def testReadFileMetadataSuccessful(self):
        with patch('lambdas.document_manager.services.dynamo_service.boto3.client', return_value=self.dynamo_client):
            from lambdas.document_manager.services.dynamo_service import read_file_metadata
...
class DeleteFileMetadataTestCase(unittest.TestCase):
    ...
    def testDeleteFileMetadataSuccessful(self):
        with patch('lambdas.document_manager.services.dynamo_service.boto3.client', return_value=self.dynamo_client):
            from lambdas.document_manager.services.dynamo_service import delete_file_metadata
